defmodule ExecutionPlane.Contracts.LowerSimulationScenario.V1 do
  @moduledoc """
  ...
  defp build(attrs) do
    ...
    %__MODULE__{
      ...
      scenario_id: Contracts.fetch_required_stringish!(attrs, :scenario_id),
  ...
  defp reject_semantic_provider_policy!(attrs) do
    if Enum.any?(@forbidden_semantic_keys, &has_key?(attrs, &1)) do
      raise ArgumentError,
            "semantic provider policy must not be owned by Execution Plane lower scenarios"
    end
  end
  ...
  defp validate_bounded_evidence_projection!(attrs) do
    ...
    unless Map.get(projection, "contract_version") == LowerSimulationEvidence.contract_version() do
      raise ArgumentError,
            "bounded_evidence_projection.contract_version must be #{LowerSimulationEvidence.contract_version()}"
    end

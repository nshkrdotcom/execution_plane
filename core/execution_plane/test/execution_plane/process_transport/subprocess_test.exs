defmodule ExecutionPlane.Process.Transport.SubprocessTest do
  ...
  test "eager startup streams stdout and a normalized exit to tagged subscribers" do
    ...
    assert_receive {:execution_plane_process, ^ref, {:message, "alpha"}}, 2_000
    assert_receive {:execution_plane_process, ^ref, {:message, "beta"}}, 2_000

    assert_receive {:execution_plane_process, ^ref,
  ...
  test "buffers early events until the first subscriber attaches when configured" do
    ...
    assert_receive {:execution_plane_process, ^ref, {:message, ^long_line}}, 2_000
    assert_receive {:execution_plane_process, ^ref, {:message, "next"}}, 2_000
  ...
  test "raw stdout mode preserves exact bytes and exposes transport metadata" do
    ...
    assert_receive {:execution_plane_process, ^ref, {:data, "alpha"}}, 2_000
  ...
  test "short-lived local subprocesses keep the shared exec worker stable across exits" do
    ...
    Enum.each([{"pipe", []}, {"pty", [pty?: true]}], fn {label, opts} ->
      ...
      assert_receive {:execution_plane_process, ^ref, {:message, "ready"}}, 2_000
  ...
  test "configured stdin interrupt payloads are written exactly to stdin" do
    ...
    assert_receive {:execution_plane_process, ^ref, {:exit, %ProcessExit{code: 130}}},
                   2_000
  end
  ...
  test "close_stdin_on_start closes stdin immediately after boot" do
    ...
    assert_receive {:execution_plane_process, ^ref, {:message, "done"}}, 2_000
  ...
  test "monitor-based subscriber cleanup keeps the transport alive until the last subscriber leaves" do
    ...
    child =
      spawn(fn ->
        ...
        receive do
          {:execution_plane_process, ^child_ref, {:message, line}} ->
    ...
    assert_receive {:execution_plane_process, ^ref, {:message, "fanout"}}, 2_000
  ...
  test "late subscribers can receive the retained stderr tail from the core" do
    ...
    assert_receive {:execution_plane_process, ^ref, {:stderr, "late stderr"}}, 2_000
    File.write!(gate_path, "release")
  ...
  test "fast-exit stderr is retained for late subscribers" do
    ...
    assert_receive {:execution_plane_process, ^ref, {:stderr, "fast stderr"}}, 2_000
  ...
  test "oversized stdout below the recoverable ceiling is chunk-recovered intact" do
    ...
    assert_receive {:execution_plane_process, ^ref, {:message, ^long_line}}, 5_000
    assert_receive {:execution_plane_process, ^ref, {:message, "after"}}, 5_000
  ...
  test "oversized stdout above the recoverable ceiling emits a fatal structured overflow" do
    ...
    refute_receive {:execution_plane_process, ^ref, {:message, "after"}}, 250
    assert :disconnected == Subprocess.status(transport)
  end
  ...
  test "large stdout fragments below the buffer limit are flushed intact on exit" do
    ...
    assert_receive {:execution_plane_process, ^ref, {:message, ^large_line}}, 5_000
  ...
  test "post-exit stdout flush preserves the trailing fragment exactly" do
    ...
    assert_receive {:execution_plane_process, ^ref, {:message, ^fragment}}, 2_000
  ...
  test "interrupt supports in-flight subprocesses and surfaces the resulting exit" do
    ...
    assert_receive {:execution_plane_process, ^ref, {:exit, %ProcessExit{} = exit}}, 2_000

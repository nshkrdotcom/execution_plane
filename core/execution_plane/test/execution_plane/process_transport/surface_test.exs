defmodule ExecutionPlane.Process.Transport.SurfaceTest do
  ...
  test "test-only guest-local helper proves path semantics are independent of remote topology" do
    assert {:ok, capabilities} = Surface.capabilities(:test_guest_local)
    assert capabilities.remote? == false
    assert capabilities.path_semantics == :guest
    assert Surface.remote_surface?(:test_guest_local) == false
    assert Surface.nonlocal_path_surface?(:test_guest_local) == true
  end

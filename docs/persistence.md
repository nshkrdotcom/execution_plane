# Execution Plane Workspace Persistence

## Scope

Execution Plane Workspace owns lower execution contract workspace persistence orientation. This document is package-local and is the persistence contract for `.` in `execution_plane`.
...
Memory/ref-only evidence by default.
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. Durable refs may be carried as evidence by higher owners after preflight.
...
Execution Plane common contracts do not own durable schema.

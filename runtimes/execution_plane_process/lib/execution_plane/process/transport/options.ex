...
defmodule ExecutionPlane.Process.Transport.Options do
  @moduledoc """
  ...
  @default_event_tag :execution_plane_process
  ...
  @default_task_supervisor ExecutionPlane.TaskSupervisor
  ...
  defp normalize_surface(opts) when is_list(opts) do
    case Surface.new(opts) do
      {:ok, surface} -> {:ok, surface}
      {:error, reason} -> {:error, reason}
    end
  end

  defp surface_metadata(%Surface{} = surface) do
    %{
